...
function throwProdutosSchemaMismatch() {
  throw new AdminProductsError(
    "A tabela 'produtos' existe mas faltam colunas que o app usa (ex.: data_criacao, precos, seo, is_featured, in_catalog). No Supabase, SQL Editor: rode `produtos-in-catalog-migration.sql` e demais migrações na raiz do repo.",
    { status: 503 },
  );
}
...
export async function listAdminProductsLocal({
  ...
  if (search) {
    query = mode === "sku" ? query.ilike("sku", `%${search}%`) : query.ilike("descricao", `%${search}%`);
  }
...
export async function getAdminCatalogOlistIds() {
  const supabase = getSupabaseOrThrow();
  const { data, error } = await supabase.from(PRODUTOS_TABLE).select("olist_id").eq("in_catalog", true);
  ...
  const ids = (data ?? [])
    .map((row) => (row?.olist_id != null ? String(row.olist_id) : ""))
    .filter(Boolean);
  return ids;
}
...
export async function replaceAdminCatalogSelection(idsInput) {
  ...
  const { error: setError } = await supabase
    ...
    .in("olist_id", numericIds);

  if (setError) {
    if (isMissingTableError(setError)) {
      throwProdutosTableMissing();
    }
    if (isSchemaMismatchError(setError)) {
      throwProdutosSchemaMismatch();
    }
    throw new AdminProductsError("Nao foi possivel atualizar o catalogo.", { status: 500, cause: setError });
  }
