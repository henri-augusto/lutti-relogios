...
export const OlistApiError = createDomainErrorClass("OlistApiError");
...
function asArrayFromPayload(payload) {
  ...
  /** Envelopes comuns do webhook Olist/Tiny (corpo fora do formato GET /produtos). */
  if (typeof payload === "object" && payload !== null) {
    const p = /** @type {Record<string, unknown>} */ (payload);
    for (const key of ["dados", "body", "payload", "content", "resultado"]) {
      const inner = p[key];
      if (inner != null && typeof inner === "object" && !Array.isArray(inner)) {
        const found = asArrayFromPayload(inner);
        if (found.length > 0) {
          return found;
        }
      }
      if (typeof inner === "string" && inner.trim()) {
        try {
          ...
          const found = asArrayFromPayload(parsedInner);
          if (found.length > 0) {
            return found;
          }
        } catch {
          /* ignora string que nao e JSON */
        }
      }
    }
  }
...
/**
 * Extrai produtos normalizados de um corpo de webhook ou JSON no formato da API Olist/Tiny.
 * @param {unknown} payload
 * @returns {NormalizedOlistProduct[]}
 */
export function normalizedProductsFromPayload(payload) {
  return asArrayFromPayload(payload)
    .map((raw) => normalizeOlistProduct(raw))
    .filter((n) => n != null);
}
