...
export const AdminPedidosError = createDomainErrorClass("AdminPedidosError");

const { getSupabaseOrThrow, rethrowSupabaseError: rethrowPedidosError } = createSupabaseDomainAccess(
  AdminPedidosError,
  {
  tableMissing:
...
export async function listAdminPedidos({ page = 1, pageSize = 50 } = {}) {
  ...
  if (error) {
    rethrowPedidosError(error, "Nao foi possivel listar pedidos.", { includeSchema: false });
  }
...
export async function verificarPedidosStripe({ days = 30, limit = 100 } = {}) {
  ...
  if (sessionIds.length > 0) {
    ...
    if (error) {
      rethrowPedidosError(error, "Nao foi possivel consultar pedidos existentes.", {
        includeSchema: false,
      });
    }
