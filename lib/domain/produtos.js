...
export function getSupabaseClient() {
  ...
  const anon = getFirstRuntimeEnv(["SUPABASE_ANON_KEY", "NEXT_PUBLIC_SUPABASE_ANON_KEY"]);
  const publishable = getFirstRuntimeEnv([
    "SUPABASE_PUBLISHABLE_KEY",
    "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY",
  ]);
  const service = getFirstRuntimeEnv(["SUPABASE_SERVICE_ROLE_KEY", "SUPABASE_SECRET_KEY"]);
  const isServer = typeof window === "undefined";
  const key = isServer ? service || anon || publishable : anon || publishable;
...
export async function getProdutos() {
  ...
  if (!supabase) {
    throw new ProductsFetchError(
      "Configure SUPABASE_URL (ou NEXT_PUBLIC_SUPABASE_URL) e SUPABASE_SERVICE_ROLE_KEY, SUPABASE_ANON_KEY ou NEXT_PUBLIC_SUPABASE_ANON_KEY no ambiente de producao.",
    );
  }
