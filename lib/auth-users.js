...
function getAuthSupabaseOrThrow() {
  ...
  fetch("http://127.0.0.1:7573/ingest/65feb5ce-7cf6-40ef-b7df-7e3cf80f3de2", {
    ...
    body: JSON.stringify({
      ...
      hypothesisId: "H1_H3",
      location: "lib/auth-users.js:getAuthSupabaseOrThrow",
      message: "Supabase client resolution",
      data: { hasSupabaseClient: Boolean(supabase) },
      timestamp: Date.now(),
    }),
  }).catch(() => {});
  // #endregion
...
export async function findUserByEmail(email) {
  ...
  const supabase = getAuthSupabaseOrThrow();
...
export async function findUserById(id) {
  const raw = typeof id === "string" ? id.trim() : id === null || id === undefined ? "" : String(id);
  if (!raw) {
    return null;
  }

  const supabase = getAuthSupabaseOrThrow();
  const { data, error } = await supabase.from(USER_TABLE).select("*").eq("id", raw).maybeSingle();
...
/**
 * Cadastro implicito no checkout (senha aleatoria). Nao verifica duplicidade de e-mail — use findUserByEmail antes.
 ...
 * @param {string} payload.neighborhood
 * @param {string} payload.city
 * @param {string} payload.state
 */
export async function insertUsuarioCheckoutConvidado(payload) {
  const supabase = getAuthSupabaseOrThrow();
  ...
  const row = {
    ...
    document:
      typeof payload.document === "string"
        ? payload.document.replace(/\D/g, "").slice(0, 14)
        : String(payload.document ?? "").replace(/\D/g, "").slice(0, 14),
    cep: normalizeCep(payload.cep),
    street: payload.street.trim(),
    number: payload.number.trim(),
    complement: typeof payload.complement === "string" ? payload.complement.trim() : "",
    neighborhood: payload.neighborhood.trim(),
    city: payload.city.trim(),
    state: payload.state.trim().toUpperCase().slice(0, 2),
  };

  const { data, error } = await supabase.from(USER_TABLE).insert(row).select("id").single();
