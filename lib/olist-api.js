...
export async function listOlistProducts({ page = 1, pageSize = 20, keyword = "" } = {}) {
  ...
  const rawItems = asArrayFromPayload(payload);
  ...
  } else if (pagination.currentPage != null && pagination.totalPages != null) {
    hasNext = pagination.currentPage < pagination.totalPages;
  } else {
    hasNext = rawItems.length >= safePageSize;
  }
  ...
  return {
    ...
    hasNext,
    totalEstimated: pagination.total,
    offset,
    limit: safePageSize,
  };
}
